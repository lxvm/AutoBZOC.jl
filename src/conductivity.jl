...
function interpolateconductivitykw(;
    ...
    return jldopen(joinpath(pwd(), "conductivitykwcache.jld2"), "a+") do fn
        id = "t$(t)_t′$(t′)_Δ($Δ)_T($T)_T₀$(T₀)_Z$(Z)_ν$(ν)_nsp$(nsp)_μlims$(μlims)_kalg$(kalg)_falg$(falg)_atol$(atol)_rtol($rtol)_Ωlims$(Ωlims)_vcomp$(vcomp)_bzkind$(bzkind)_natol$(natol)_nrtol$(nrtol)_nalg$(nalg)"
        if !haskey(fn, id)
            μ = findchempot(; io=io, verb=verb, t=t, t′=t′, Δ=Δ, T=T, T₀=T₀, Z=Z, μlims=μlims, ν=ν, nsp=nsp, alg=nalg, kalg=kalg, falg=falg, atol=natol, rtol=nrtol, bzkind=bzkind)
            ...
            abstol = atol/10 # tighter tolerance for integration than interpolation
            reltol = rtol/10 # tighter tolerance for integration than interpolation
            ...
            f = BatchFunction() do x
                ...
                batchsolve(solver, x, SMatrix{3,3,typeof(complex(atol)),9}, nthreads=Threads.nthreads())
            end
...
function interpolateconductivityk(;
    ...
    return jldopen(joinpath(pwd(), "conductivitykcache.jld2"), "a+") do fn
        id = "t$(t)_t′$(t′)_Δ($Δ)_Ω$(Ω)_T($T)_T₀$(T₀)_Z$(Z)_ν$(ν)_nsp$(nsp)_μlims$(μlims)_kalg$(kalg)_falg$(falg)_atol$(atol)_rtol($rtol)_ωlims$(ωlims)_vcomp$(vcomp)_bzkind$(bzkind)_natol$(natol)_nrtol$(nrtol)_nalg$(nalg)_μoffset$(μoffset)"
        if !haskey(fn, id)
            μ = findchempot(; io=io, verb=verb, t=t, t′=t′, Δ=Δ, T=T, T₀=T₀, Z=Z, μlims=μlims, ν=ν, nsp=nsp, alg=nalg, kalg=kalg, falg=falg, atol=natol, rtol=nrtol, bzkind=bzkind)
            ...
            abstol = atol/10 # tighter tolerance for integration than interpolation
            reltol = rtol/10
            integrand = OpticalConductivityIntegrand(bz, kalg, w, Σ, β, Ω, μoffset, abstol=abstol/det(bz.B)/nsyms(bz), reltol=reltol)
            f = BatchFunction() do x
                status(length(x))
...
#     bzkind=CubicSymIBZ(), nalg=Falsi(), natol=1e-5, nrtol=1e-5, io=stdout, verb=true)
    ...
    return jldopen(joinpath(pwd(), "conductivitywcache.jld2"), "a+") do fn
        id = "t$(t)_t′$(t′)_Δ($Δ)_Ω$(Ω)_T($T)_T₀$(T₀)_Z$(Z)_ν$(ν)_nsp$(nsp)_μlims$(μlims)_kalg$(kalg)_falg$(falg)_atol$(atol)_rtol($rtol)_klims$(klims)_vcomp$(vcomp)_bzkind$(bzkind)_natol$(natol)_nrtol$(nrtol)_nalg$(nalg)"
        if !haskey(fn, id)
            μ = findchempot(; io=io, verb=verb, t=t, t′=t′, Δ=Δ, T=T, T₀=T₀, Z=Z, μlims=μlims, ν=ν, nsp=nsp, alg=nalg, kalg=kalg, falg=falg, atol=natol, rtol=nrtol, bzkind=bzkind)
